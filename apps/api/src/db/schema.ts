import {
  pgTable, pgEnum, serial, varchar, boolean, date,
  integer, numeric, text, jsonb, char, timestamp,
  check, primaryKey, AnyPgColumn, unique
...
export const minuteVersionStatus = pgEnum('minute_version_status', ['aguardando aprovação', 'aprovada', 'substituída'])
export const eventType = pgEnum('event_type', ['culto', 'reunião', 'evento especial', 'outro'])
export const recurrenceType = pgEnum('recurrence_type', ['nenhuma', 'semanal', 'quinzenal', 'mensal'])
export const closingStatus = pgEnum('closing_status',['aberto', 'em revisão', 'rejeitado', 'aprovado', 'fechado'])
...
export const roleModulePermissions = pgTable('role_module_permissions', {
  ...
  primaryKey({ columns: [table.roleId, table.moduleId, table.permissionId] })
])
...
export const userModulePermissions = pgTable('user_module_permissions', {
  ...
  primaryKey({ columns: [table.userId, table.moduleId, table.permissionId] })
])
...
export const paymentMethods = pgTable('payment_methods', {
  ...
  check('chk_at_least_one_flow', sql`${table.allowsInflow} = true OR ${table.allowsOutflow} = true`)
])
...
export const expenseEntries = pgTable('expense_entries', {
  ...
  check('chk_expense_installments_valid', sql`${table.installment} > 0 AND ${table.totalInstallments} > 0 AND ${table.installment} <= ${table.totalInstallments}`)
])

export const treasuryClosings = pgTable('treasury_closings', {
  id: serial('id').primaryKey(),
...
export const minutes = pgTable('minutes', {
  id: serial('id').primaryKey(),
  boardMeetingId: integer('board_meeting_id').unique().notNull().references(() => boardMeetings.id),
...
export const minuteVersions = pgTable('minute_versions', {
  id: serial('id').primaryKey(),
  minuteId: integer('minute_id').notNull().references(() => minutes.id, { onDelete: 'cascade' }),
