...
Font.register({
  family: 'NotoSans',
  fonts: [
    { src: path.join(FONTSOURCE, 'noto-sans/files/noto-sans-latin-100-normal.woff'), fontWeight: 100 }, // font-thin
    { src: path.join(FONTSOURCE, 'noto-sans/files/noto-sans-latin-200-normal.woff'), fontWeight: 200 }, // font-extralight
    { src: path.join(FONTSOURCE, 'noto-sans/files/noto-sans-latin-300-normal.woff'), fontWeight: 300 }, // font-light
    { src: path.join(FONTSOURCE, 'noto-sans/files/noto-sans-latin-400-normal.woff'), fontWeight: 400 }, // font-normal
    { src: path.join(FONTSOURCE, 'noto-sans/files/noto-sans-latin-500-normal.woff'), fontWeight: 500 }, // font-medium
    { src: path.join(FONTSOURCE, 'noto-sans/files/noto-sans-latin-600-normal.woff'), fontWeight: 600 }, // font-semibold
    { src: path.join(FONTSOURCE, 'noto-sans/files/noto-sans-latin-700-normal.woff'), fontWeight: 700 }, // font-bold
    { src: path.join(FONTSOURCE, 'noto-sans/files/noto-sans-latin-800-normal.woff'), fontWeight: 800 }, // font-extrabold
    { src: path.join(FONTSOURCE, 'noto-sans/files/noto-sans-latin-900-normal.woff'), fontWeight: 900 }, // font-black
  ],
});
...
function PageHeader({ period, title = 'Relatório de Fechamento Simples' }: { period: { from: string; to: string }; title?: string }) {
  return (
    <View style={tw('px-8 pt-8 pb-6 mb-6 border-b border-gray-200')}>
      <View style={tw('flex-row justify-between items-end mb-2')}>
        <View>
          ...
          <Text style={tw('text-3xl font-roboto font-bold text-slate-800 tracking-tight leading-none')}>
            {title}
          </Text>
        </View>

        <View style={tw('bg-slate-50 border border-gray-200 rounded px-3 py-2')}>
          <Text style={tw('text-[0.625rem] leading-tight font-roboto font-bold text-zinc-600 tracking-widest mb-0.5')}>
            PERÍODO DE APURAÇÃO
          </Text>
...
function PageFooter() {
  return (
    <View
      style={tw('absolute bottom-5 left-8 right-8 flex-row justify-between pt-1 border-t border-gray-200')}
      fixed
    >
  ...
  return (
    <View>
      <View style={tw('flex-row bg-gray-100 border-b border-gray-200')}>
        ...
        <Text style={[tw('px-2 py-1.5 text-xs font-roboto font-medium'), { flex: 3 }]}>Categoria</Text>
        <Text style={[tw('px-2 py-1.5 text-xs font-roboto font-medium text-right'), { flex: 1 }]}>Total</Text>
      </View>
      ...
      {rows.map((r, i, a) => {
        ...
        return (
          <View key={r.categoryId} style={tw(rowClasses)}>
            <Text style={[tw('px-2 py-1.5 text-xs text-slate-700'), { flex: 2 }]}>{r.parentCategoryName ?? '—'}</Text>
            <Text style={[tw('px-2 py-1.5 text-xs text-slate-700'), { flex: 3 }]}>{r.categoryName}</Text>
            <Text style={[tw('px-2 py-1.5 text-xs text-slate-700 text-right'), { flex: 1 }]}>{fmtCurrency(r.total)}</Text>
          </View>
        );
      })}

      <View style={tw(type === 'income' ? 'flex-row bg-emerald-50 border-t border-green-200' : 'flex-row bg-red-50 border-t border-red-100')}>
        <Text style={[tw(type === 'income' ? 'px-2 py-1.5 text-xs font-medium font-roboto text-teal-900' : 'px-2 py-1.5 text-xs font-medium font-roboto text-red-900'), { flex: 5 }]}>{totalLabel}</Text>
        <Text style={[tw(type === 'income' ? 'px-2 py-1.5 text-xs font-medium font-roboto text-right text-teal-900' : 'px-2 py-1 text-xs font-medium font-roboto text-right text-red-900'), { flex: 2 }]}>{fmtCurrency(total)}</Text>
      </View>
    </View>
  );
}

function SummaryCards({ openingBalance, totalIncome, totalExpenses, currentBalance }: {
  ...
  return (
    <View style={tw('flex-row mb-5')}>
      <View style={tw('flex-1 border border-slate-200 rounded p-2.5 mr-2')}>
        ...
        <Text style={getBalanceStyle(openingBalance)}>
          {fmtCurrency(openingBalance)}
        </Text>
      </View>
      <View style={tw('flex-1 border border-slate-200 rounded p-2.5 mr-2')}>
        <Text style={tw('text-xs font-roboto font-medium mb-1')}>TOTAL ENTRADAS</Text>
        <Text style={tw('text-sm font-roboto text-slate-700 font-medium')}>{fmtCurrency(totalIncome)}</Text>
      </View>
      <View style={tw('flex-1 border border-slate-200 rounded p-2.5 mr-2')}>
        <Text style={tw('text-xs font-roboto font-medium mb-1')}>TOTAL SAÍDAS</Text>
        <Text style={tw('text-sm font-roboto text-slate-700 font-medium')}>{fmtCurrency(totalExpenses)}</Text>
      </View>
  ...
  return (
    <View wrap={false} style={tw('px-8 mt-auto')}>
      ...
      <Text style={tw('text-xs text-gray-500 leading-normal')}>
        Declaramos que os presentes demonstrativos financeiros foram examinados e estão em conformidade com os
        registros contábeis da Igreja Batista Nova Jerusalém referentes ao período indicado.
      </Text>
    </View>
  );
}
...
function IncomeDetailTable({ pivot }: { pivot: IncomePivot }) {
  ...
  return (
    <View>
      <View style={tw('flex-row bg-gray-100 border-b border-gray-200')}>
        <Text style={[tw('px-2 py-1.5 text-xs font-roboto font-medium'), { flex: dateColFlex }]}>Data</Text>
        {pivot.columns.map((col) => (
          <Text key={col.key} style={[tw('px-2 py-1.5 text-xs font-roboto font-medium text-right'), { flex: valColFlex }]}>{col.label}</Text>
        ))}
        <Text style={[tw('px-2 py-1.5 text-xs font-roboto font-medium text-right'), { flex: totalColFlex }]}>Total</Text>
      </View>
      ...
      {pivot.rows.map((row, i) => {
        ...
        return (
          <View key={row.referenceDate} style={tw(['flex-row', border, bg].filter(Boolean).join(' '))}>
            <Text style={[tw('px-2 py-1.5 text-xs text-slate-700'), { flex: dateColFlex }]}>{fmtDate(row.referenceDate)}</Text>
            {pivot.columns.map((col) => (
              <Text key={col.key} style={[tw('px-2 py-1.5 text-xs text-slate-700 text-right'), { flex: valColFlex }]}>
                {fmtCurrency(row.cells[col.key] ?? '0.00')}
              </Text>
            ))}
            <Text style={[tw('px-2 py-1.5 text-xs text-slate-700 font-medium text-right'), { flex: totalColFlex }]}>{fmtCurrency(row.total)}</Text>
          </View>
        );
      })}

      <View style={tw('flex-row bg-emerald-50 border-t border-green-200')}>
        <Text style={[tw('px-2 py-1.5 text-xs font-medium font-roboto text-teal-900'), { flex: dateColFlex }]}>Total</Text>
        {pivot.columns.map((col) => (
          <Text key={col.key} style={[tw('px-2 py-1.5 text-xs font-medium font-roboto text-right text-teal-900'), { flex: valColFlex }]}>
            {fmtCurrency(col.total)}
          </Text>
        ))}
        <Text style={[tw('px-2 py-1.5 text-xs font-medium font-roboto text-right text-teal-900'), { flex: totalColFlex }]}>{fmtCurrency(pivot.grandTotal)}</Text>
      </View>
    </View>
  );
}
  ...
  return (
    <View>
      <View style={tw('flex-row bg-gray-100 border-b border-gray-200')}>
        ...
        <Text style={[tw('px-2 py-1.5 text-xs font-roboto font-medium'), { flex: 2 }]}>Grupo</Text>
        <Text style={[tw('px-2 py-1.5 text-xs font-roboto font-medium'), { flex: 3 }]}>Categoria</Text>
        <Text style={[tw('px-2 py-1.5 text-xs font-roboto font-medium'), { flex: 4 }]}>Descrição</Text>
        <Text style={[tw('px-2 py-1.5 text-xs font-roboto font-medium text-right'), { flex: 1.1 }]}>Valor</Text>
      </View>
      {rows.map((r, i, a) => {
        ...
        return (
          <View key={r.id} style={tw(['flex-row', border, bg].filter(Boolean).join(' '))}>
            <Text style={[tw('px-2 py-1.5 text-xs text-slate-700'), { flex: 0.9 }]}>{fmtDate(r.referenceDate)}</Text>
            <Text style={[tw('px-2 py-1.5 text-xs text-slate-700'), { flex: 2 }]}>{r.parentCategoryName ?? '—'}</Text>
            <Text style={[tw('px-2 py-1.5 text-xs text-slate-700'), { flex: 3 }]}>{r.categoryName}</Text>
            <Text style={[tw('px-2 py-1.5 text-xs text-slate-700'), { flex: 4 }]}>{r.description}</Text>
            <Text style={[tw('px-2 py-1.5 text-xs text-slate-700 text-right'), { flex: 1.1 }]}>{fmtCurrency(r.amount)}</Text>
          </View>
        );
      })}
      <View style={tw('flex-row bg-red-50 border-t border-red-100')}>
        <Text style={[tw('px-2 py-1.5 text-xs font-medium font-roboto text-red-900'), { flex: 9.9 }]}>Total</Text>
        <Text style={[tw('px-2 py-1.5 text-xs font-medium font-roboto text-right text-red-900'), { flex: 1.1 }]}>{fmtCurrency(total)}</Text>
      </View>
    </View>
  );
}

export function DetailedFinancialStatementPdf({ data }: { data: DetailedFinancialStatementResponse }) {
  return (
    <Document>
