...
export async function list(req: FastifyRequest, reply: FastifyReply) {
  const { page, limit, status } = req.query as PaginationQuery & { status?: FundStatusValue };
...
export async function remove(req: FastifyRequest, reply: FastifyReply) {
  const { id } = req.params as IdParam;
  const result = await service.softDeleteDesignatedFund(req.session.userId!, id);
...
export async function restore(req: FastifyRequest, reply: FastifyReply) {
  const { id } = req.params as IdParam;
  const fund = await service.restoreDesignatedFund(req.session.userId!, id);
  if (!fund) return reply.code(404).send({ message: 'Designated fund not found' });
  return reply.send(fund);
}
...
export async function encerrar(req: FastifyRequest, reply: FastifyReply) {
  const { id } = req.params as IdParam;
  const fund = await service.encerrarDesignatedFund(req.session.userId!, id);
  if (!fund) return reply.code(404).send({ message: 'Designated fund not found' });
  return reply.send(fund);
}
...
export async function reabrir(req: FastifyRequest, reply: FastifyReply) {
  const { id } = req.params as IdParam;
  const fund = await service.reabrirDesignatedFund(req.session.userId!, id);
  if (!fund) return reply.code(404).send({ message: 'Designated fund not found' });
  return reply.send(fund);
}
