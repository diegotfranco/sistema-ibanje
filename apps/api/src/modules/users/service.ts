...
export async function listUsers(page: number, limit: number) {
  ...
  return {
    data: rows.map((row): UserResponse => ({
      id: row.id,
      name: row.name,
      email: row.email,
      role: row.roleName,
...
export async function changePassword(callerId: number, targetId: number, body: UpdatePasswordRequest) {
  if (callerId === targetId) {
...
export async function setUserPermissions(callerId: number, targetId: number, permissionsMap: Record<string, string[]>) {
  if (callerId === targetId) {
    throw httpError(403, 'Cannot modify your own permissions')
  }

  const { rows, unknownModules, unknownPermissions } = await repo.resolvePermissionRows(permissionsMap)
...
export async function createUser(body: CreateUserRequest, logFn: (token: string) => void): Promise<UserResponse> {
...
export async function approveUser(targetId: number, logFn: (token: string, email: string) => void): Promise<UserResponse | null> {
  return await db.transaction(async (tx) => {
