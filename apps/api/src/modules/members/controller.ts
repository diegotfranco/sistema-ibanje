...
import type { CreateMemberRequest, UpdateMemberRequest } from './schema.js';
...
export async function create(req: FastifyRequest, reply: FastifyReply) {
  ...
  logAudit(req.session.userId!, 'create', 'member', member.id, { ipAddress: req.ip });
...
export async function update(req: FastifyRequest, reply: FastifyReply) {
  ...
  if (!member) return reply.code(404).send({ message: 'Member not found' });
  logAudit(req.session.userId!, 'update', 'member', id, { ipAddress: req.ip });
  return reply.send(member);
}
...
export async function remove(req: FastifyRequest, reply: FastifyReply) {
  ...
  logAudit(req.session.userId!, 'delete', 'member', id, { ipAddress: req.ip });
